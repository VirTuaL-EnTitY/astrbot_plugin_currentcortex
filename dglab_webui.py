...
4. 用手机 App 扫描二维码完成配对（v3 与 v4 App 的接入方式略有不同，见下）
5. 配对成功后即可通过指令或 CCDG WebUI 控制

### v3 APP（DG-LAB 3.0 App）

- 适用于 DG-LAB 3 App（兼容旧版 App），中转服务器端口一般为 9999
- 控制端裸连服务器根路径，如 \`ws://host:9999\`
- 服务器分配配对 ID 后生成二维码（dungeon-lab.com/app-download.php 开头），App 扫码后以该 ID 连接 \`ws://host:9999/{配对ID}\` 完成配对
- 强度与波形经标准消息帧（type 1/2/3/4/clientMsg）转发

### v4 APP（DG-LAB 4.0 App）

- 适用于 DG-LAB 4 App，中转服务器端口一般为 9998，可带路径前缀（如 \`wss://host:9998/v4\`）
- 连接后服务器分配配对 ID，生成新版二维码（dungeon-lab.cn/s/ 开头）；**该二维码仅 DG-LAB 4 App 可识别**，旧版 App 无法扫码配对
- App 内可连接多台设备（多插槽），插件会自动选择第一个真实连接的设备插槽
- 控制经 RPC 帧下发，设备与强度状态由 App 主动上报同步
