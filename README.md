...
| 🧩 **按群聊开关** | 在单个群用 `/开关` 命令关闭/开启本插件全部命令（支持限时关闭自动恢复），互不影响 |
| 🧠 **跨群聊记忆** | 同平台所有群共享一份持久化上下文，自动注入 LLM 请求（可按时效过滤、按关键词清理） |
...
| `/开关` | `/toggle` `/switch` | 按群聊开关本插件全部命令（支持限时） |
| `/开关列表` | `/switch_list` `/开关状态列表` | 查看本平台被关闭的群（管理员） |
...
| `/开关 status`（或 `/开关 状态`） | 查看本群当前状态（限时关闭会显示预计恢复时间） |
| `/开关列表`（`/switch_list` `/开关状态列表`） | **管理员**查看当前平台所有被关闭的群及各自恢复时间 |
| `/开关` | 无参数 = 查看状态 + 用法提示 |
...
/开关 status    # 查看状态：⛔ 已关闭（⏳ 将于 9小时58分钟后 自动恢复）
/开关列表       # 管理员：查看本平台所有被关闭的群
...
- **限时自动恢复**：`/开关 off <时长>` 到期后自动恢复启用（懒惰过期判断，无需后台定时任务），重启后倒计时依然有效。
