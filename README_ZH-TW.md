...
Pixiv 隨機圖片 · 每日一言 · 天氣查詢 · 男娘圖片 · 網易雲點歌 · 小紅書／B站／抖音解析 · DG-LAB 裝置管理 · 跨群聊記憶 · LLM 工具（AI 自主呼叫）· 語意分段回覆
...
| 🔍 **媒體解析** | 小紅書圖文影片 · B站影片資訊 · 抖音無浮水印影片 |
| 📚 **媒體內容解析** | 小紅書 / B站 / 抖音連結解析 |
...
| 🧩 **按群聊開關** | 在單一群組用 `/开关` 指令一鍵關閉／開啟本插件全部指令，互不影響 |
| 🧠 **跨群聊記憶** | 同平台所有群組共享一份持久化上下文，自動注入 LLM 請求 |
...
自動辨識平台並解析小紅書、B站、抖音的媒體連結，回傳無浮水印圖片／影片資訊。
...
| 指令 | 說明 |
| --- | --- |
| `/开关 off`（或 `/开关 关`） | **關閉**本群全部插件指令（pixiv／解析／music／… 均不再回應） |
| `/开关 on`（或 `/开关 开`） | **重新啟用**本群插件指令 |
| `/开关 status`（或 `/开关 状态`） | 查看本群目前狀態 |
| `/开关` | 無參數＝查看狀態＋用法提示 |

> 別名：`/toggle`、`/switch`（如 `/toggle off`）。

#### 使用範例

```text
/开关 off       # 在本群關閉 CurrentCortex 全部指令
/pixiv          # 此時不再回應（已關閉）
/开关 status    # 查看狀態：⛔ 已關閉
/开关 on        # 重新啟用
/pixiv          # 恢復正常
...
- **永不死鎖**：`/开关` 指令本身**始終可用**——即使本群已關閉，仍可傳送 `/开关 on` 重新啟用，不會被攔截。
- **權限**：預設僅**群組管理員**（框架識別的 admin）可操作，避免任意成員隨意開關。若你未被識別為管理員，可在設定中關閉 `group_switch_admin_only`。
...
- **注入**：群組訊息觸發 LLM 請求時，自動把同平台其他群的最近若干筆記錄以 `<system_reminder>` 注入使用者訊息部分。
- **斜線指令不記錄**：指令訊息不會進入記憶。
