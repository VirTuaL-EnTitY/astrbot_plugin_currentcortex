...
| 🧩 **按群聊開關** | 在單一群組用 `/开关` 指令關閉／開啟本插件全部指令（支援限時關閉自動恢復），互不影響 |
| 🧠 **跨群聊記憶** | 同平台所有群共享一份持久化上下文，自動注入 LLM 請求（可依時效過濾、依關鍵字清理） |
...
| `/开关` | `/toggle` `/switch` | 按群聊開關本插件全部指令（支援限時） |
| `/开关列表` | `/switch_list` `/开关状态列表` | 查看本平台被關閉的群（管理員） |
...
| `/开关 status`（或 `/开关 状态`） | 查看本群目前狀態（限時關閉會顯示預計恢復時間） |
| `/开关列表`（`/switch_list` `/开关状态列表`） | **管理員**查看目前平台所有被關閉的群及各自恢復時間 |
| `/开关` | 無參數＝查看狀態＋用法提示 |
...
/开关 status    # 查看狀態：⛔ 已關閉（⏳ 將於 9小時58分鐘後 自動恢復）
/开关列表       # 管理員：查看本平台所有被關閉的群
...
- **限時自動恢復**：`/开关 off <時長>` 到期後自動恢復啟用（惰性過期判斷，無需背景定時任務），重新啟動後倒數依然有效。
- **永不死鎖**：`/开关` 與 `/开关列表` 指令本身**始終可用**——即使本群已關閉，仍可傳送 `/开关 on` 重新啟用，不會被攔截。
- **權限**：預設僅**群組管理員**（框架識別的 admin）可操作，避免任意成員隨意開關。若你未被識別為管理員，可在設定中關閉 `group_switch_admin_only`。`/开关列表` 始終僅管理員可用。
